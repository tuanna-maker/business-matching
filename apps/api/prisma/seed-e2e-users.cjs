...
async function main() {
  ...
  // 7. Seed IEC Levels (required for assessment tests)
  console.log("\nSeeding IEC Levels...");
  const iecLevels = [
    { level: "L1", name: "Level 1 - Concept", description: "Early concept stage verification" },
    { level: "L2", name: "Level 2 - Validation", description: "Market validation and early traction" },
    { level: "L3", name: "Level 3 - Growth", description: "Growth-ready with proven model" }
  ];
  for (const lvl of iecLevels) {
    const existing = await prisma.iecLevel.findFirst({ where: { level: lvl.level } });
    if (!existing) {
      await prisma.iecLevel.create({ data: lvl });
      console.log(`  ✅ IEC Level: ${lvl.level} - ${lvl.name}`);
    } else {
      console.log(`  ⚠️  IEC Level ${lvl.level} already exists`);
    }
  }
